...
lulc_2020 <- project(lulc_2020, target_crs, method = "near")
lulc_2018 <- project(lulc_2018, target_crs, method = "near")
lulc_2014 <- project(lulc_2014, target_crs, method = "near")
lulc_2010 <- project(lulc_2010, target_crs, method = "near")
...
pts_in_village <- st_join(sacred_points, 
                          village_clean_1 %>%
                            select(net_income_adjusted, 
                                   popu_density,
                                   agri_land_pp), 
                          left = FALSE)
...
final_10000_pts_12345_500 <- select_stratified_points(
  study_area = village_clean,
  lulc_raster = lulc_2002,
  target_lulc = c(1, 2, 3, 4, 5),  # 在这里更改点下的 LULC 类型
  exclude_polygons_list = list(pa, buffers_1000m), # 放入所有要剔除的层
  initial_size = 30000,
  final_size = 10000
)
...
sacred_true_points_500 <- sacred_points %>%
  right_join(final_buffers_true_500 %>% 
             ...
             by = "ID") %>%
  mutate(is_sns = 1) %>%
  st_join(village_clean_1[, c("net_income_adjusted", "popu_density", "agri_land_pp")]) %>%
  select("net_income_adjusted", "popu_density", "agri_land_pp", "is_sns", "民族", "丧葬类型", "有无寺庙") %>%
  rename(x = geometry,
         ethnicity = 民族,
         burial = 丧葬类型,
         temple = 有无寺庙) %>%
  ...
  filter(!(dai == 1 & temple == "无"))


sacred_false_points_12345_500 <- final_10000_pts_12345_500 %>%
  mutate(is_sns = 0) %>%
  st_join(village_clean[, c("net_income_adjusted", "popu_density", "agri_land_pp", "dai", "hani", "bulang")]) %>%
  select("net_income_adjusted", "popu_density", "agri_land_pp", "dai", "hani", "is_sns", "bulang") %>%
  filter(dai == 1 | hani == 1 | bulang == 1)
